...
func TestSystemDicKOPath(t *testing.T) {
	expected := "dic"
	if KoDicPath != expected {
		t.Errorf("got %v, expected %v", KoDicPath, expected)
	}
}

func TestSystemDicMorphs(t *testing.T) {
	d := SysDicKO()
	const expected = KoDicEntrySize
	c := len(d.Morphs)
	if c != expected {
...
func TestSystemDicContents(t *testing.T) {
	const expected = KoDicEntrySize
	got := len(SysDicKO().Contents)
	if got != expected {
		t.Errorf("got %v, expected %v", got, expected)
	}
}

func TestSystemDicCharClass(t *testing.T) {
	expected := []string{
		"DEFAULT",      // 0
		"SPACE",        // 1
		"HANJA",        // 2
	...
	if !reflect.DeepEqual(SysDic().CharClass, expected) {
		t.Errorf("got %v, expected %v", SysDic().CharClass, expected)
	}
}

func TestSysDicCharCategoryMap(t *testing.T) {
	const (
		DEFAULT = iota
		SPACE
		HANJA
...
func TestSysDicCharCategorySize(t *testing.T) {
...
func TestSysDicInvokeList(t *testing.T) {
	const (
		DEFAULT = iota
		SPACE
		HANJA
...
func TestSysDicGroupList(t *testing.T) {
	const (
		DEFAULT = iota
		SPACE
		HANJA
...
func BenchmarkSysDicKO(b *testing.B) {
	for i := 0; i < b.N; i++ {
		loadInternalSysDicFull(KoDicPath)
	}
}

func BenchmarkSysDicKOSimple(b *testing.B) {
	for i := 0; i < b.N; i++ {
		loadInternalSysDicSimple(KoDicPath)
	}
}
